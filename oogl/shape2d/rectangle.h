...
class rect4draw : public DrawUnit {
  public:
    rect4draw(std::shared_ptr<Window> win, std::shared_ptr<Texture> texture,
              const Size &size);
    rect4draw(std::shared_ptr<Window> win, const std::string &texturePath,
              const Size &size);

    rect4draw(std::shared_ptr<Window> win, std::shared_ptr<Texture> texture,
              const Size &size, const rectangle &loadPos);
    rect4draw(std::shared_ptr<Window> win, const std::string &texturePath,
              const Size &size, const rectangle &loadPos);
