...
namespace oogl {
	...
	namespace shape2d
	{
		class DrawUnit {
		public:
			DrawUnit() : m_translation(0, 0, 0), m_rotation(0.0f),
				m_scale(1.0f, 1.0f, 1.0f), m_transparency(1.0f) {}
			virtual ~DrawUnit() {}
		public:
		...
		{
		private:
			virtual void onDraw(const glm::vec3& translation, float rotationZ, const glm::vec3& scale, float transparency = 1.0f) = 0;
		public:
			Drawable() {}
			virtual ~Drawable() {};
			void draw(const DrawUnit& da) { draw(da.getTranslation(), da.getRotationAngle(), da.getScale(), da.getTransparency()); }
			void draw(const glm::vec3& translation, float rotation, const glm::vec3& scale, float transparency) {
