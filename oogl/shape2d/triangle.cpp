...
namespace oogl {
	namespace shape2d
	{
		...
		void triangle4draw::onDraw(const glm::vec3& translation, float rotationZ, const glm::vec3& scale, float transparency)
		{
			getShader().SetUniform1f("c_Transparency", transparency);
			getTexture().Bind();
			auto envScale = getWindow().getScale();
			glm::mat4 model = glm::translate(glm::mat4(1.0f), {translation.x * envScale.x ,translation.y * envScale.y, translation.z});
			glm::mat4 mvp = getWindow().getDrawProj() * getWindow().getDrawView() * model;
			mvp = glm::rotate(mvp, glm::radians(rotationZ), glm::vec3(0.0f, 0.0f, 1.0f));
		...
		triangle4draw::triangle4draw(std::shared_ptr <Window> win, std::shared_ptr<Texture> texture, const triangle& showPos, const triangle& loadPos): m_VAO()
		{
			init(win, texture, showPos, loadPos);
		}
		triangle4draw::triangle4draw(std::shared_ptr <Window> win, const std::string& texturePath, const triangle& showPos, const triangle& loadPos): m_VAO()
		{
		...
		void triangle4draw::setShowPos(const triangle& showPos)
		{
			...
			this->setVertexBuffer(std::make_shared<VertexBuffer>(positions, 4 * 3 * sizeof(float)));
		...
		void triangle4draw::init(std::shared_ptr<Window> win, std::shared_ptr<Texture> texture, const triangle& showPos, const triangle& loadPos)
		{
			m_loadPos = loadPos;
			this->setWindow(win);
