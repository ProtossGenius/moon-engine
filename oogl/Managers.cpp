...
template <typename Type> class OManager : public Manager<Type> {
  ...
  public:
    ...
    void free(const std::string &path) override { m_tstore.erase(path); }

  private:
    Store m_tstore;
};

template <typename Type> Manager<Type> &getManager() {
    static OManager<Type> mgr;
    return mgr;
}

Manager<Texture> &getTextureMgr() { return getManager<Texture>(); }

Manager<Shader> &getShaderMgr() { return getManager<Shader>(); }
