...
namespace oogl {
	...
	Window::Window(int width, int height, const char* title, GLFWmonitor* monitor, GLFWwindow* share)
		:m_drawProj(glm::ortho(0.0f, float(width), 0.0f, float(height), -1.0f, 1.0f)),
		m_drawView(glm::translate(glm::mat4(1.0f), glm::vec3(0, 0, 0))),
		m_scale(1.0f, 1.0f, 1.0f),
		m_swapInterval(1), m_window(nullptr), m_width(width), m_height(height),
		m_designWidth(width), m_designHeight(height), m_KeyMouse(smgame::nil<KeyMouseItf>())
	{
		InitGlfw();
		...
		glfwSetKeyCallback(m_window, [](GLFWwindow* win, int key, int scancode, int action, int mode) {
			auto window = winRegs[uint64_t(win)];
			window->KeyBoards(key, scancode, action, mode);
			});

		glfwSetScrollCallback(m_window, [](GLFWwindow* win, double, double offsetY) {
			auto window = winRegs[uint64_t(win)];
	...
	void Window::resize(int width, int height) {
		...
		m_drawProj = glm::ortho(0.0f, float(width), 0.0f, float(height), -1.0f, 1.0f);
		m_scale = { float(width) / float(m_designWidth), float(height) / float(m_designHeight), 1.0f };
