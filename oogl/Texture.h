...
union ARGB_UNION {
    ...
    ARGB_UNION(int argb)
        : argb(argb) {}
    ARGB_UNION(int8_t a, int8_t r, int8_t g, int8_t b)
        : arr{b, g, r, a} {}
};
...
class ARGB {
  public:
    ...
    ARGB(int argb)
        : m_argbUnion(argb) {}
    ARGB(int8_t a, int8_t r, int8_t g, int8_t b)
        : m_argbUnion{a, r, g, b} {}
...
inline int distance(const ARGB &lhs, const ARGB &rhs) {
...
class Texture {
  ...
  public:
    Texture(const std::string &path)
        : Texture(path, false, 0, 0) {}
    Texture(const std::string &path, bool do_trans, ARGB trans_color,
            int trans_distance);
    ~Texture();
