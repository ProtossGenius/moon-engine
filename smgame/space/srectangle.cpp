...
namespace smgame {
	namespace x2d {
		Rectangle::Rectangle(const Point& p1, const Point& p2):
			m_ZeroPoint(min(p1.x, p2.x), max(p1.y, p2.y)), m_FlagPoint(max(p1.x, p2.x), min(p1.y, p2.y)){}

		Rectangle Rectangle::suit(const Rectangle& rhs) const
		{
			return Rectangle({min(max(m_ZeroPoint.x, rhs.m_ZeroPoint.x), rhs.m_FlagPoint.x), 
		...
		void Rectangle::rangeCoorrdinateLine(std::function<void(const CoordinateLine&)> act)const
		{
			act({ CoordinateLineType::ParrallelX,m_ZeroPoint.y, m_ZeroPoint.x, m_FlagPoint.x });
			act({ CoordinateLineType::ParrallelY,m_ZeroPoint.x, m_ZeroPoint.y, m_FlagPoint.y });
			act({ CoordinateLineType::ParrallelX,m_FlagPoint.y, m_ZeroPoint.x, m_FlagPoint.x });
			act({ CoordinateLineType::ParrallelY,m_FlagPoint.x, m_ZeroPoint.y, m_FlagPoint.y });
		}

		bool Rectangle::Impact(const Point& p) const
		{
			return in_range(p.x, m_ZeroPoint.x, m_FlagPoint.x) && in_range(p.y, m_FlagPoint.y, m_ZeroPoint.y);
		}
		...
		{
			bool isImpact = false;
			r1.rangeCoorrdinateLine([&isImpact, &r2](const CoordinateLine& l) {	if (r2.Impact(l)) isImpact = true;	});
			if (isImpact)return true;
			r2.rangeCoorrdinateLine([&isImpact, &r1](const CoordinateLine& l) {	if (r1.Impact(l)) isImpact = true;	});
			return isImpact;
		}
