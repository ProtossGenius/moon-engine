...
namespace smgame {
	namespace x2d {
		...
		public:
			...
			LengthUnit Redis;
		public:
			bool Impact(const Point& p) const { return ::smgame::distance(Center, p) < Redis; }
			bool Impact(const CoordinateLine& line)const;
			bool Impact(const Circle& c) const {return ::smgame::distance(Center, c.Center) < (Redis + c.Redis);}
		};
