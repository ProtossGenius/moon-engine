...
template <typename Type> inline bool in_range(Type n, Type start, Type end) {
    using namespace std;
    if (start > end) {
        swap(start, end);
    }
...
template <typename Type> inline bool out_range(Type n, Type start, Type end) {
...
template <typename Type> inline Type square(Type t) { return t * t; }
} // namespace smgame