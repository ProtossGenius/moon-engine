...
class WorldMap : public world_itf {
  public:
    ...
    ~WorldMap() {
        if (m_terrains) delete[] m_terrains;
    }
  ...
  public:
    ...
    void
         rangeInArea(const x2d::Rectangle &                                 area,
                     std::function<void(Terrain &, const x2d::Rectangle &)> act);
    void userEvent(const UnitID                             uid,
                   const std::list<std::shared_ptr<Event>> &events) override;
    // (0, 0) is leftDown;
