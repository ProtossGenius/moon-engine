...
class AttributesItf : public Serializable {
  ...
  public:
    AttributesItf(int attribType)
        : AttribType(attribType)
        , m_changeDo(doNothing) {}

  public:
    virtual AttributesItf &
        parseMsg(const std::string &) override     = 0; // return itself.
    ...
    template <typename Attrib> typename Attrib::Trait::Type getAttrib() const {
        if (Attrib::AttribType != AttribType) {
            throw SMGAME_ERROR(
                "Attrib not suit. Except AttribType is: " +
                std::to_string(AttribType) +
                ", Use AttribID is : " + std::to_string(Attrib::AttribType) +
                ", AttribID = " + std::to_string(Attrib::AttribID) +
                ", template Attrib's Name is " + typeid(Attrib).name());
        }
